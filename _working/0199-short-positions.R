...
extremes <- most_shorted %>%
  ...
  arrange(date) %>%
  select(product, product_code, short_position_prop, date)
...
all_stocks_data %>%
  inner_join(all_data, by = c("date", "product_code")) %>%
  ggplot(aes(x = volume, y = short_position_prop, colour = product_code)) +
  geom_path() +
  geom_smooth(aes(colour = NULL), method = "gam", colour = "black") +
  scale_y_log10(label = percent) +
...
all_data %>%
  ...
  filter(n > 1) %>%
  inner_join(distinct(all_data, product_code, product)) %>%
  View
...
all_stocks_data %>%
  ...
  ggplot(aes(x = volume / 1000, y = short_position_prop, colour = date)) +
  geom_path() +
  geom_smooth(method = "gam", colour = "black") +
  facet_wrap(~recent_product_name, scales = "free") +
  scale_y_log10(label = percent) +
