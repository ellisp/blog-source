...
Of the two visualisations I probably prefer that last one, the heat map. First, it dramatically (all that white) shows that the regression estimates from the true model aren't biased at all. Secondly, it nicely shows that the bias in the estimates returned by stepwise regression are worse
- for smaller samples
